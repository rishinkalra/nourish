...
Privacy-minimized structured API and worker telemetry now records templated routes, status, duration, queue delay, retry/dead outcomes, bounded failure codes, and validated opaque correlation IDs. Correlation identity propagates across the client/API boundary into plan, notification, privacy, and subscription jobs. The DigitalOcean template adds restart, CPU, and memory alerts; selecting a monitoring destination and activating uptime, latency, log-derived queue/error/webhook alerts plus responder routing remain account-level staging work.
