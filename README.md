...
It is not yet production-ready. PostgreSQL runtime adapters now cover consumer and administrator identity, profiles, durable recipe author/review/publication, verified StoreKit/App Store notification ingress and scheduled Apple status reconciliation, asynchronous plan generation/history/adoption, immutable confirmed swaps with compatible weekly-state preservation, feedback, grocery/prep/meal state, privacy requests, controlled admin exports, scheduled physical export cleanup, and leased workers. Memory mode remains the local development default, while production startup now rejects ephemeral persistence and unsafe configuration. A locked container, separate migration job, schema-aware readiness, encrypted S3-compatible private storage, staging harness, smoke check, and deployment/recovery method define the next live-validation step. Live database/bucket policy and backup-retention validation, production email, workforce identity-provider configuration, approved StoreKit product/credential/App Store Connect configuration and sandbox validation, licensed professionally reviewed nutrition data and ingestion, and physical-device background validation remain.
