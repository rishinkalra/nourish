...
Run `swift run NourishCoreChecks` from this folder to verify the shared domain layer. The shared `NourishApp` scheme also includes `NourishAppTests` and `NourishAppUITests`; use Xcode’s Test action on an iPhone Simulator to run the twelve XCTest cases and eighteen end-to-end UI journeys together.
