...
- The existing responsive web experience is packaged and validated as an explicitly labelled sample-data staging preview. A private hosted version is active, and `staging.familychef.in` is attached pending its CNAME and two ownership-validation TXT records. Public access remains an owner approval; authenticated API-backed profiles and plans remain gated on transactional email configuration.
